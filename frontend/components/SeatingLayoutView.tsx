"use client";

import React, { useState, useEffect } from "react";
import {
  Building2,
  Layers,
  LayoutGrid,
  Plus,
  RefreshCw,
  Info,
  Check,
  ChevronRight,
  SlidersHorizontal,
  MapPin,
  Monitor,
} from "lucide-react";
import { api, Venue, VenueLayout, AreaLayout, VenueSection, PhysicalSeat } from "../lib/api";

interface SeatingLayoutViewProps {
  venues: Venue[];
  selectedVenueId?: string;
  onSelectVenue: (id: string) => void;
  onOpenCreateArea: (venueId: string) => void;
  onOpenCreateSection: (venueId: string, areaId?: string) => void;
  isAdminLoggedIn: boolean;
}

export const SeatingLayoutView: React.FC<SeatingLayoutViewProps> = ({
  venues,
  selectedVenueId,
  onSelectVenue,
  onOpenCreateArea,
  onOpenCreateSection,
  isAdminLoggedIn,
}) => {
  const [currentVenueId, setCurrentVenueId] = useState<string>(
    selectedVenueId || (venues.length > 0 ? venues[0].id : "")
  );
  const [layout, setLayout] = useState<VenueLayout | null>(null);
  const [selectedAreaId, setSelectedAreaId] = useState<string>("");
  const [selectedSeats, setSelectedSeats] = useState<{ id: string; code: string }[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    if (selectedVenueId) {
      setCurrentVenueId(selectedVenueId);
    } else if (venues.length > 0 && !currentVenueId) {
      setCurrentVenueId(venues[0].id);
    }
  }, [selectedVenueId, venues]);

  const loadLayout = async (vId: string) => {
    if (!vId) return;
    setLoading(true);
    setError(null);
    setSelectedSeats([]);

    try {
      const data = await api.getVenueLayout(vId);
      setLayout(data);
      if (data.areas && data.areas.length > 0) {
        setSelectedAreaId(data.areas[0].id);
      } else {
        setSelectedAreaId("");
      }
    } catch (err: any) {
      setError(err.message || "Failed to load layout");
      setLayout(null);
    } finally {
      setLoading(false);
    }
  };

  useEffect(() => {
    if (currentVenueId) {
      loadLayout(currentVenueId);
    }
  }, [currentVenueId]);

  const currentVenue = venues.find((v) => v.id === currentVenueId);
  const activeArea = layout?.areas.find((a) => a.id === selectedAreaId);

  const toggleSeat = (seatId: string, code: string) => {
    setSelectedSeats((prev) => {
      const exists = prev.some((s) => s.id === seatId);
      if (exists) {
        return prev.filter((s) => s.id !== seatId);
      } else {
        return [...prev, { id: seatId, code }];
      }
    });
  };

  return (
    <div className="w-full flex flex-col space-y-4">
      {/* Top Bar / Venue Switcher */}
      <div className="bg-white dark:bg-[#181a24] p-4 border border-zinc-200 dark:border-zinc-800 shadow-sm flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <Building2 className="w-5 h-5 text-[#f84464]" />
          <div>
            <span className="text-xs font-bold uppercase tracking-wider text-zinc-500 dark:text-zinc-400 block">
              Active Venue
            </span>
            <select
              value={currentVenueId}
              onChange={(e) => {
                setCurrentVenueId(e.target.value);
                onSelectVenue(e.target.value);
              }}
              className="font-bold text-base text-zinc-900 dark:text-zinc-100 bg-transparent border-b border-zinc-300 dark:border-zinc-700 pb-0.5 outline-none focus:border-[#f84464]"
            >
              {venues.map((v) => (
                <option key={v.id} value={v.id} className="bg-white dark:bg-[#1a1c24] text-zinc-900 dark:text-zinc-100">
                  {v.name} ({v.city})
                </option>
              ))}
            </select>
          </div>
        </div>

        {currentVenue && (
          <div className="flex items-center gap-3 text-xs text-zinc-500 dark:text-zinc-400">
            <span className="flex items-center gap-1">
              <MapPin className="w-3.5 h-3.5 text-[#f84464]" />
              {currentVenue.address}, {currentVenue.city}
            </span>
            <button
              onClick={() => loadLayout(currentVenueId)}
              className="p-1.5 border border-zinc-300 dark:border-zinc-700 hover:text-[#f84464] transition-colors"
              title="Refresh Layout"
            >
              <RefreshCw className={`w-3.5 h-3.5 ${loading ? "animate-spin text-[#f84464]" : ""}`} />
            </button>
          </div>
        )}
      </div>

      {/* Screen / Area Tabs (BookMyShow Multi-Screen Style) */}
      {layout && layout.areas && layout.areas.length > 0 && (
        <div className="bg-white dark:bg-[#181a24] border border-zinc-200 dark:border-zinc-800 p-2 flex flex-wrap items-center justify-between gap-2">
          <div className="flex items-center gap-1.5 overflow-x-auto">
            <span className="text-xs font-bold uppercase tracking-wider text-zinc-400 px-2 flex items-center gap-1">
              <Monitor className="w-3.5 h-3.5" />
              Screens:
            </span>
            {layout.areas.map((area) => (
              <button
                key={area.id}
                onClick={() => setSelectedAreaId(area.id)}
                className={`px-3.5 py-1.5 text-xs font-bold tracking-wide uppercase transition-colors border flex items-center gap-1.5 ${
                  selectedAreaId === area.id
                    ? "bg-[#f84464] text-white border-[#f84464]"
                    : "border-zinc-200 dark:border-zinc-700 hover:border-zinc-400 dark:hover:border-zinc-500 text-zinc-700 dark:text-zinc-300 bg-zinc-50 dark:bg-zinc-800/30"
                }`}
              >
                <span>{area.name}</span>
                <span className="text-[10px] opacity-80">({area.sections.length} sections)</span>
              </button>
            ))}
          </div>

          <div className="flex items-center gap-2 ml-auto">
            <button
              onClick={() => onOpenCreateArea(currentVenueId)}
              className="px-2.5 py-1 text-xs bg-zinc-100 dark:bg-zinc-800 hover:bg-[#f84464] hover:text-white border border-zinc-300 dark:border-zinc-700 transition-colors font-medium flex items-center gap-1 text-zinc-700 dark:text-zinc-300"
            >
              <Plus className="w-3 h-3" />
              <span>Add Screen</span>
            </button>
            <button
              onClick={() => onOpenCreateSection(currentVenueId, selectedAreaId)}
              className="px-2.5 py-1 text-xs bg-[#f84464] hover:bg-[#e51a4b] text-white transition-colors font-bold flex items-center gap-1"
            >
              <Plus className="w-3 h-3" />
              <span>Add Section</span>
            </button>
          </div>
        </div>
      )}

      {/* Main Layout Area */}
      {loading ? (
        <div className="bg-white dark:bg-[#181a24] p-16 border border-zinc-200 dark:border-zinc-800 text-center text-zinc-500 dark:text-zinc-400 space-y-2">
          <RefreshCw className="w-8 h-8 animate-spin mx-auto text-[#f84464]" />
          <p className="text-sm font-semibold uppercase tracking-wider">Loading physical seating layout...</p>
        </div>
      ) : error ? (
        <div className="bg-white dark:bg-[#181a24] p-8 border border-red-300 dark:border-red-900/40 text-center text-red-500 space-y-2">
          <p className="text-sm font-bold">{error}</p>
          <button
            onClick={() => loadLayout(currentVenueId)}
            className="px-4 py-1.5 text-xs bg-[#f84464] text-white font-bold uppercase"
          >
            Retry
          </button>
        </div>
      ) : !layout || layout.areas.length === 0 ? (
        /* Empty State: No Areas */
        <div className="bg-white dark:bg-[#181a24] p-12 border border-zinc-200 dark:border-zinc-800 text-center space-y-4">
          <Layers className="w-12 h-12 mx-auto text-zinc-300 dark:text-zinc-700" />
          <div className="space-y-1">
            <h3 className="text-base font-bold uppercase text-zinc-800 dark:text-zinc-200">
              No Screens or Areas Defined for this Venue
            </h3>
            <p className="text-xs text-zinc-500 max-w-md mx-auto">
              Real venues contain multiple screens/auditoriums (e.g. Screen 1, IMAX, Gold Class). Add your first screen to start configuring sections and physical seats.
            </p>
          </div>
          <button
            onClick={() => onOpenCreateArea(currentVenueId)}
            className="px-5 py-2 bg-[#f84464] hover:bg-[#e51a4b] text-white text-xs font-bold uppercase tracking-wider transition-colors inline-flex items-center gap-1.5"
          >
            <Plus className="w-4 h-4" />
            <span>Create First Screen / Area</span>
          </button>
        </div>
      ) : !activeArea || activeArea.sections.length === 0 ? (
        /* Empty State: Area has no sections */
        <div className="bg-white dark:bg-[#181a24] p-12 border border-zinc-200 dark:border-zinc-800 text-center space-y-4">
          <LayoutGrid className="w-12 h-12 mx-auto text-zinc-300 dark:text-zinc-700" />
          <div className="space-y-1">
            <h3 className="text-base font-bold uppercase text-zinc-800 dark:text-zinc-200">
              No Sections in "{activeArea?.name}"
            </h3>
            <p className="text-xs text-zinc-500 max-w-md mx-auto">
              Define seating tiers (e.g. VIP Recliner, Prime, Classic) with row and seat counts to automatically generate physical seats.
            </p>
          </div>
          <button
            onClick={() => onOpenCreateSection(currentVenueId, selectedAreaId)}
            className="px-5 py-2 bg-[#f84464] hover:bg-[#e51a4b] text-white text-xs font-bold uppercase tracking-wider transition-colors inline-flex items-center gap-1.5"
          >
            <Plus className="w-4 h-4" />
            <span>Add Section & Generate Seats</span>
          </button>
        </div>
      ) : (
        /* Full BookMyShow Seating Grid */
        <div className="bg-white dark:bg-[#181a24] border border-zinc-200 dark:border-zinc-800 p-4 sm:p-8 space-y-8 shadow-sm">
          {/* Area Description & Legend */}
          <div className="flex flex-wrap items-center justify-between gap-4 border-b border-zinc-200 dark:border-zinc-800 pb-4">
            <div>
              <h2 className="text-lg font-black uppercase tracking-wide text-zinc-900 dark:text-zinc-100 flex items-center gap-2">
                <span>{activeArea.name}</span>
                {activeArea.description && (
                  <span className="text-xs font-normal text-zinc-500 dark:text-zinc-400 capitalize">
                    - {activeArea.description}
                  </span>
                )}
              </h2>
              <span className="text-xs text-zinc-500 dark:text-zinc-400">
                {activeArea.sections.length} Sections | Total Physical Seats:{" "}
                {activeArea.sections.reduce((acc, s) => acc + s.seats.length, 0)}
              </span>
            </div>

            {/* Legend */}
            <div className="flex items-center gap-4 text-xs font-medium text-zinc-600 dark:text-zinc-400">
              <div className="flex items-center gap-1.5">
                <div className="w-4 h-4 border border-emerald-500 bg-white dark:bg-[#12141c]" />
                <span>Available</span>
              </div>
              <div className="flex items-center gap-1.5">
                <div className="w-4 h-4 bg-[#f84464] text-white text-[8px] flex items-center justify-center font-bold">
                  ✓
                </div>
                <span>Selected</span>
              </div>
            </div>
          </div>

          {/* Sections Display */}
          <div className="space-y-8 overflow-x-auto py-2">
            {activeArea.sections.map((section) => {
              // Group seats by rowLabel
              const rowMap = new Map<string, PhysicalSeat[]>();
              for (const seat of section.seats) {
                if (!rowMap.has(seat.rowLabel)) {
                  rowMap.set(seat.rowLabel, []);
                }
                rowMap.get(seat.rowLabel)!.push(seat);
              }

              return (
                <div key={section.id} className="space-y-3">
                  {/* Section Title Header (BMS Tier Header) */}
                  <div className="border-b border-zinc-200 dark:border-zinc-800 pb-1 flex items-center justify-between text-xs">
                    <span className="font-extrabold uppercase tracking-widest text-[#f84464]">
                      {section.name} SECTION
                    </span>
                    <span className="font-mono text-[11px] text-zinc-500">
                      {section.rowsCount} Rows × {section.seatsPerRow} Seats ({section.seats.length} Seats)
                    </span>
                  </div>

                  {/* Seat Grid Rows */}
                  <div className="space-y-2 py-1 min-w-[320px]">
                    {Array.from(rowMap.entries()).map(([rowLabel, seatsInRow]) => (
                      <div key={rowLabel} className="flex items-center justify-center gap-3 sm:gap-6">
                        {/* Left Row Label */}
                        <span className="w-5 text-center font-mono font-bold text-xs text-zinc-400 select-none">
                          {rowLabel}
                        </span>

                        {/* Seats in this row */}
                        <div className="flex items-center gap-1.5 sm:gap-2">
                          {seatsInRow.map((seat) => {
                            const seatCode = `${seat.rowLabel}${seat.seatNumber}`;
                            const isSelected = selectedSeats.some((s) => s.id === seat.id);

                            return (
                              <button
                                key={seat.id}
                                onClick={() => toggleSeat(seat.id, seatCode)}
                                className={`w-7 h-7 sm:w-8 sm:h-8 flex items-center justify-center font-mono text-[10px] sm:text-xs font-bold transition-all select-none cursor-pointer border ${
                                  isSelected
                                    ? "bg-[#f84464] text-white border-[#f84464] shadow-xs"
                                    : "border-emerald-600/70 hover:border-[#f84464] hover:bg-[#f84464]/10 text-zinc-800 dark:text-zinc-200 bg-white dark:bg-[#14151e]"
                                }`}
                                title={`Seat ${seatCode}`}
                              >
                                {seat.seatNumber}
                              </button>
                            );
                          })}
                        </div>

                        {/* Right Row Label */}
                        <span className="w-5 text-center font-mono font-bold text-xs text-zinc-400 select-none">
                          {rowLabel}
                        </span>
                      </div>
                    ))}
                  </div>
                </div>
              );
            })}
          </div>

          {/* BookMyShow Screen Arc Representation */}
          <div className="pt-8 pb-4 space-y-3 max-w-md mx-auto text-center">
            <div className="w-full h-2 bg-gradient-to-r from-transparent via-cyan-400 to-transparent shadow-[0_4px_12px_rgba(34,211,238,0.4)]" />
            <p className="text-[11px] font-bold uppercase tracking-widest text-zinc-400 dark:text-zinc-500">
              All eyes this way please (Screen)
            </p>
          </div>

          {/* Interactive Selection Footer */}
          {selectedSeats.length > 0 && (
            <div className="sticky bottom-4 z-30 bg-[#333545] dark:bg-[#14151e] text-white p-3.5 border border-white/20 shadow-2xl flex flex-wrap items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                <div className="bg-[#f84464] text-white px-2.5 py-1 text-xs font-bold">
                  {selectedSeats.length} {selectedSeats.length === 1 ? "Seat" : "Seats"} Selected
                </div>
                <div className="text-xs font-mono font-bold text-zinc-300 truncate max-w-md">
                  {selectedSeats.map((s) => s.code).join(", ")}
                </div>
              </div>

              <div className="flex items-center gap-2 ml-auto">
                <button
                  onClick={() => setSelectedSeats([])}
                  className="px-3 py-1 text-xs text-zinc-300 hover:text-white border border-zinc-600 hover:border-zinc-400 transition-colors"
                >
                  Clear Selection
                </button>
              </div>
            </div>
          )}
        </div>
      )}
    </div>
  );
};
